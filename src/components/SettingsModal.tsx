import type { AppSettings, GraphicsQuality } from "../types/arcade";
...
export const SettingsModal = ({ open, settings, onChange, onClose }: SettingsModalProps): React.JSX.Element | null => {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-[rgba(3,9,20,0.72)] p-4 backdrop-blur-md" role="presentation">
      <section role="dialog" aria-modal="true" aria-label="Global settings" className="settings-dialog w-full max-w-2xl rounded-[1.8rem] border border-sky-100/14 bg-[rgba(7,18,34,0.96)] p-6 text-slate-50 shadow-[0_32px_72px_-38px_rgba(0,0,0,0.88)]">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="font-display text-3xl text-white">Settings</h2>
          <button type="button" onClick={onClose} className="arcade-btn-secondary px-4 py-2">Close</button>
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-amber-200/18 bg-amber-300/8 p-3 md:col-span-2">
            ...
            <p className="mt-2 text-xs text-sky-50/70">Higher quality sharpens the 3D games but costs more GPU. Low is the safest setting, and games will ease render resolution down if FPS drops.</p>
          </div>
        </div>
      </section>
    </div>
...
interface ToggleRowProps { label: string; checked: boolean; onChange: (checked: boolean) => void; }
...
const ToggleRow = ({ label, checked, onChange }: ToggleRowProps): React.JSX.Element => (
  <label className="flex items-center justify-between rounded-2xl border border-sky-100/12 bg-white/5 px-4 py-3 text-sky-50">
    <span>{label}</span>
    <button type="button" role="switch" aria-checked={checked} onClick={() => onChange(!checked)} className={`relative h-7 w-12 rounded-full border ${checked ? "border-amber-200/50 bg-amber-300" : "border-sky-100/16 bg-white/10"}`}>
